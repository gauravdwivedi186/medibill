...
const Page = () => {
  ...
  async function handleDownload(orderID) {
    try {
      const response = await fetch(`/api/invoice?id=${orderID}`);
  ...
  const totalAmount = filteredProducts.reduce((acc, curr) => acc + parseFloat(curr.Total || 0), 0);
    ...
    <>
      <div class="p-4 mx-auto container mt-5">
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg h-screen">
          ...
          <span className="text-lg font-semibold mr-4">Total Amount: ₹{totalAmount.toFixed(2)}</span>
                ...
                {filteredProducts.map((Product) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <button
                        onClick={() => handleDownload(Product.OrderID)}
                        className="font-medium text-blue-600 hover:underline"
                      >
                        BILL
                      </button>
